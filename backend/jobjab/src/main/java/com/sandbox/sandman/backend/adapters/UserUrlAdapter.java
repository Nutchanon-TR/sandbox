...
@Component
public class UserUrlAdapter implements JobSourceAdapter {
    private final HtmlJobExtractor htmlJobExtractor;
    private final HttpClient httpClient;

    public UserUrlAdapter(HtmlJobExtractor htmlJobExtractor) {
    ...
    public JobFetchMode fetchMode() {
        return JobFetchMode.USER_URL;
    }

    @Override
    public List<ExternalJobRef> search(JobSearchCriteria criteria) {
        if (criteria == null) return List.of();
        String title = criteria.query() == null || criteria.query().isBlank()
                ? "Imported job"
                : criteria.query().trim();
        return urls(criteria).stream()
                .limit(criteria.limit() == null ? 10 : Math.max(1, criteria.limit()))
                .map(url -> new ExternalJobRef(AdapterSupport.canonicalJobUrl(url), url, title, "Unknown company", criteria.locationText()))
                .toList();
    }
    ...
    public RawJobPayload fetchDetail(ExternalJobRef ref) {
        ...
        payload.put("importMode", "USER_PASTED_URL");
        try {
    ...
    public NormalizedJobRecord normalize(RawJobPayload raw) {
        ...
        String url = AdapterSupport.canonicalJobUrl(string(payload.get("url")));
        String host = hostOf(url);
        String title = fallback(string(payload.get("title")), "Imported job from " + host);
        String company = fallback(string(payload.get("company")), host);
        String location = fallback(string(payload.get("locationText")), "Location not provided");
        return new NormalizedJobRecord(
                ...
                fallback(string(payload.get("description")), "Imported from a user-provided URL. Add JD text in a later extraction pass for deeper AI matching."),
                AdapterSupport.canonicalJobUrl(fallback(string(payload.get("finalUrl")), url)),
                null
        );
    }
    ...
    private List<String> stringList(Object value) {
        if (value instanceof List<?> list) {
            return list.stream().filter(item -> item != null && !String.valueOf(item).isBlank()).map(String::valueOf).distinct().toList();
        }
        return List.of();
    }
