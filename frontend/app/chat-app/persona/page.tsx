...
export default function PersonaFeedPage() {
    ...
    const [devPublishingPersonaId, setDevPublishingPersonaId] = useState<number | null>(null);
    ...
    const handleDevPublishNow = async (personaId: number) => {
        setDevPublishingPersonaId(personaId);
        try {
            await fetchApi<void>(
            ...
            setDevPublishingPersonaId(null);
        }
    };
                ...
                <div className="grid gap-4">
                    {posts.map((post) => (
                        <PersonaFeedPostCard
                            ...
                            devPublishing={devPublishingPersonaId === post.persona.id}
                        />
                    ))}
                </div>
            )}
